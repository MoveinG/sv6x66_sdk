...
static void print_timestamp(void)
{
	...
	ms = OS_GetSysTick();
	sprintf(timestr, "[%06d.%03d]", (int)(ms/1000), (int)(ms%1000));

	drv_uart_write_fifo((unsigned char*)timestr, 12, UART_BLOCKING);
}
...
int printf(const char* format, ...)
{
	...
	for (i = 0; i < ret; ++i) 
	{
		if(buffer[i] == '\n')
		{
			if(i>j) drv_uart_write_fifo((unsigned char*)buffer+j, i-j, UART_BLOCKING);
			j = i+1;
	...
	if(j < ret)
		drv_uart_write_fifo((unsigned char*)buffer+j, ret-j, UART_BLOCKING);
