...
extern void ada_demo_main(void);
extern void demo_init(void);
int At_AlyaLedevb (s32 argc, char *argv[])
{
    ada_demo_main();
    return ERROR_SUCCESS;
}
...
int At_LedTest (s32 argc, char *argv[]){
    change_led_val(strtoul(argv[0],NULL,10));

    printf("Set to led val:%d\n",strtoul(argv[0],NULL,10));
...
CLICmds gAlCliCmdTable[21];

void cli_register(const char *cmd, const char *help,
		void (*handler)(int argc, char **argv))
{
	static int idx = 0;
    if(idx == 0){
        memset(gAlCliCmdTable,0,sizeof(gAlCliCmdTable));
    }
	if(idx >= 20)
		return;
	gAlCliCmdTable[idx].Cmd  = cmd;
	gAlCliCmdTable[idx].CmdHandler = handler;
	gAlCliCmdTable[idx].CmdUsage = help;
	idx++;
}
