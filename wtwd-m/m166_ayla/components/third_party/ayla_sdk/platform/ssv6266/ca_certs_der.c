/*
 * Copyright 2017 Ayla Networks, Inc.  All rights reserved.
 */
#include <stdlib.h>
#include <platform/pfm_net_tls.h>
#include "mbedtls/ssl.h"
#include "mbedtls/entropy.h"
#include "mbedtls/ctr_drbg.h"
#include "mbedtls/timing.h"
#include "mbedtls/x509_crt.h"
#include "mbedtls/net.h"
#include "mbedtls/debug.h"
#include "ayla/log.h"
/*
 * entrust-ca-g2-sha256-2048.der
 * Issuer:
 *   C=US
 *   O=Entrust, Inc.
 *   OU=See www.entrust.net
 *   legal-terms
 *   OU=(c) 2009 Entrust, Inc. - for authorized use only
 *   CN=Entrust Root Certification Authority - G2
 * Subject:
 *   C=US
 *   O=Entrust, Inc.
 *   OU=See www.entrust.net
 *   legal-terms
 *   OU=(c) 2009 Entrust, Inc. - for authorized use only
 *   CN=Entrust Root Certification Authority - G2
 * notBefore:
 *    Jul  7 17:25:54 2009 GMT
 * notAfter:
 *    Dec  7 17:55:54 2030 GMT
*/
static const unsigned char entrust_ca_g2_sha256_2048[] __attribute__((section(".user_data"))) = {
  ,
};
/*
 * entrust-ca-sha1-2048-sn-0x456b5054.der
 * Issuer:
 *   C=US
 *   O=Entrust, Inc.
 *   OU=www.entrust.net
 *   CPS is incorporated by reference
 *   OU=(c) 2006 Entrust, Inc.
 *   CN=Entrust Root Certification Authority
 * Subject:
 *   C=US
 *   O=Entrust, Inc.
 *   OU=www.entrust.net
 *   CPS is incorporated by reference
 *   OU=(c) 2006 Entrust, Inc.
 *   CN=Entrust Root Certification Authority
 * notBefore:
 *    Nov 27 20:23:42 2006 GMT
 * notAfter:
 *    Nov 27 20:53:42 2026 GMT
*/
static const unsigned char entrust_ca_sha1_2048_sn_0x456b50[] __attribute__((section(".user_data"))) = {
  ,
};
/*
 * valicert-cl2-sha1-1024.der
 * Issuer:
 *   L=ValiCert Validation Network
 *   O=ValiCert, Inc.
 *   OU=ValiCert Class 2 Policy Validation Authority
 *   CN=http:
 *   www.valicert.com
 *   emailAddress=info@valicert.com
 * Subject:
 *   L=ValiCert Validation Network
 *   O=ValiCert, Inc.
 *   OU=ValiCert Class 2 Policy Validation Authority
 *   CN=http:
 *   www.valicert.com
 *   emailAddress=info@valicert.com
 * notBefore:
 *    Jun 26 00:19:54 1999 GMT
 * notAfter:
 *    Jun 26 00:19:54 2019 GMT
*/
static const unsigned char valicert_cl2_sha1_1024[] __attribute__((section(".user_data"))) = {
  ,
};
/*
 * Baltimore_CyberTrust_Root.der
 * Issuer:
 *   C=IE
 *   O=Baltimore
 *   OU=CyberTrust
 *   CN=Baltimore CyberTrust Root
 * Subject:
 *   C=IE
 *   O=Baltimore
 *   OU=CyberTrust
 *   CN=Baltimore CyberTrust Root
 * notBefore:
 *    May 12 18:46:00 2000 GMT
 * notAfter:
 *    May 12 23:59:00 2025 GMT
*/
static const unsigned char Baltimore_CyberTrust_Root[] __attribute__((section(".user_data"))) =  {
  ,
};
/*
 * DigiCertGlobalRootCA.der
 * Issuer:
 *   C=US
 *   O=DigiCert Inc
 *   OU=www.digicert.com
 *   CN=DigiCert Global Root CA
 * Subject:
 *   C=US
 *   O=DigiCert Inc
 *   OU=www.digicert.com
 *   CN=DigiCert Global Root CA
 * notBefore:
 *    Nov 10 00:00:00 2006 GMT
 * notAfter:
 *    Nov 10 00:00:00 2031 GMT
*/
static const unsigned char DigiCertGlobalRootCA[] __attribute__((section(".user_data"))) = {
  ,
};
const struct ca_certs_der_t
{
	const char *name;
	const unsigned char *cert;
	size_t size;
} ca_certs_der[] = {
  {
    "entrust-ca-g2-sha256-2048.der",
    entrust_ca_g2_sha256_2048,
    sizeof(entrust_ca_g2_sha256_2048)
  },
  {
    "entrust-ca-sha1-2048-sn-0x456b5054.der",
    entrust_ca_sha1_2048_sn_0x456b50,
    sizeof(entrust_ca_sha1_2048_sn_0x456b50)
  },
  {
    "valicert-cl2-sha1-1024.der",
    valicert_cl2_sha1_1024,
    sizeof(valicert_cl2_sha1_1024)
  },
  {
    "Baltimore_CyberTrust_Root.der",
    Baltimore_CyberTrust_Root,
    sizeof(Baltimore_CyberTrust_Root)
  },
  {
    "DigiCertGlobalRootCA.der",
    DigiCertGlobalRootCA,
    sizeof(DigiCertGlobalRootCA)
  },
	{ NULL, NULL, 0 },
};

static mbedtls_x509_crt ca_certs_chain;
static int ca_certs_inited = 0;

mbedtls_x509_crt *load_ca_cert(void){
  struct ca_certs_der_t *der_cert = ca_certs_der;
	int err;

  if(ca_certs_inited)
    goto SUCCESS_RETURN; 

	mbedtls_x509_crt_init(&ca_certs_chain);
	while (der_cert->name) {
    err = mbedtls_x509_crt_parse_der(&ca_certs_chain,
        der_cert->cert, der_cert->size);
    if (!err) {
      ca_certs_inited = 1;
      client_log(LOG_DEBUG "Load \"%s\" successfully",
          der_cert->name);
    } else {
      client_log(LOG_ERR
          "Load \"%s\" failed, errcode=-0x%x",
          der_cert->name, -err);
    }
    der_cert++;
	}

SUCCESS_RETURN:
  return &ca_certs_chain;
  
}

void free_ca_cert(void){
  if(ca_certs_inited){
    mbedtls_x509_crt_free( &ca_certs_chain );
    ca_certs_inited = 0;
  }
    
}


