...
void client_ota_cleanup(struct client_state *state)
{
	...
	al_os_mem_free(state->ota.version);
...
static int client_lanota_hdr_chk(char *inbuf, int sz)
{
	...
	pub_key = ada_conf.pubkey;
	pub_key_len = ada_conf.pubkey_len;
#endif /* CLIENT_OTA_LAN_TEST_KEY */
	...
	if (rc < 0 || strcmp(ada_conf.dsn, dsn)) {
		goto hdr_chk_fail;
	}
	...
	state->ota.version = al_os_mem_calloc(strlen(ver) + 1);
