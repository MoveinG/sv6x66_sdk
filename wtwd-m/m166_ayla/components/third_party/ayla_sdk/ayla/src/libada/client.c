...
static struct al_ada_callback client_cb[CCB_COUNT];
...
void client_connectivity_update(void)
{
	...
	client_callback_enqueue(state->callback_queue[CQP_HIGH],
...
void client_clock_set(u32 new_time, enum al_clock_src src)
{
	u32 now;
	char buf[24];
...
static int client_set_time(struct xml_state *sp, int argc, char **argv)
{
	...
	if (!client_parse_u32(&val, argc, argv)) {
		client_clock_set(val, AL_CS_SERVER);
	}
	return 0;
}
...
static int client_oem(struct xml_state *sp, int argc, char **argv)
{
	...
	if (argc != 1 || strcmp(argv[0], ada_conf.oem)) {
...
static int client_oem_model(struct xml_state *sp, int argc, char **argv)
{
	...
	if (argc != 1 || strcmp(argv[0], ada_conf.model)) {
		state->client_info_flags |= CLIENT_UPDATE_OEM;
	}
	return 0;
}
...
client_accept_id(struct xml_state *sp, int argc, char **argv)
{
	...
	al_ada_timer_set(&state->listen_timer, CLIENT_LISTEN_WARN_WAIT);
...
static int client_ans_cipher_key(struct xml_state *sp, int argc, char **argv)
{
	...
	len = sizeof(buf);
	if (ayla_base64_decode(*argv, strlen(*argv), buf, &len)) {
...
static void client_start_polling(struct client_state *state)
{
	...
	if (!timer_active(&state->poll_timer) && delay) {
		...
		al_ada_timer_set(&state->poll_timer, delay);
	}
}
...
repeat:
	...
	for (cbqp = state->callback_queue;
		...
		cb = client_callback_dequeue(*cbqp);
		if (cb) {
	...
	if (!state->np_up && state->conn_state == CS_WAIT_EVENT) {
		...
		state->poll_ads = 0;
		al_ada_timer_cancel(&state->poll_timer);
	}
}
...
save:
	al_ada_call(&client_cb[CCB_LANIP]);
	return 0;
}
#endif /* ADA_BUILD_LAN_SUPPORT */
...
void client_tcp_recv_done(struct client_state *state)
{
	...
	state->current_request = NULL;
	state->retries = 0;
	state->connect_time = al_clock_get_total_ms();
	state->request = CS_IDLE;
					...
					void *buf, size_t len)
{
	...
	/* allocated in client_rev_rest_put() */
	hc->body_buf = NULL;
...
static void client_notify_cb(void *arg)
{
	...
	case NS_EV_DOWN_RETRY:
		...
		if (!state->poll_ads) {
			...
			al_net_dns_delete_host(hc->host);
		}
ansdown:
		if (state->np_up) {
...
{
	al_ada_timer_cancel(&state->req_timer);	/* in case of reset/commit */
	...
	if (delay) {
		al_ada_timer_set(&state->req_timer, delay);
	}
}
					...
					void *buf, size_t len)
{
	...
	if (CLIENT_HAS_KEY(state)) {
		...
		if (conf_was_reset) {
			conf_was_reset = 0;
			conf_save(CONF_PNAME_SYS_RESET);
		}
		client_logging(1);
...
static void client_get_dev_id(void *arg)
{
	...
	client_log(LOG_INFO "get DSN %s", ada_conf.dsn);
	...
	snprintf(req, sizeof(req) - 1, "/dsns/%s.xml", ada_conf.dsn);
...
enum ada_err client_recv_cmds(struct http_client *hc, void *payload, size_t len)
{
	...
	al_ada_timer_cancel(&state->cmd_timer);
					...
					void *payload, size_t len)
{
	...
	if (payload) {
		...
		if (hc->http_time) {
			client_clock_set(hc->http_time, AL_CS_HTTP);
		}
		state->ping_time = 0;
		if (CLIENT_HAS_KEY(state)) {
			client_server_conn_up(state, hc);
					...
					char *buf, size_t buf_len)
{
	...
	if (flags & CLIENT_UPDATE_OEM) {
		xml_len += snprintf(buf + xml_len, buf_len - xml_len,
		    ...
		    ada_conf.oem, ada_conf.model);
		xml_len += snprintf(buf + xml_len, buf_len - xml_len,
		    "<oem-key>");
		outlen = buf_len - xml_len;
		if (ayla_base64_encode(ada_conf.oemkey,
		    sizeof(ada_conf.oemkey), buf + xml_len, &outlen)) {
...
static void client_err_cb(struct http_client *hc)
{
	...
	if (state->conn_state == CS_WAIT_GET) {
		...
		if (state->get_cmds_fail) {
			 ...
			 */
			al_ada_timer_set(&state->cmd_timer,
	...
	case HC_ERR_HTTP_STATUS:
		...
		if (hc->http_state.status == HTTP_STATUS_UNAUTH &&
			...
			if (hc->server_time) {
				client_clock_set(hc->server_time, AL_CS_SERVER);
...
retry:
		if (state->conn_state == CS_WAIT_CONN) {
			...
			al_net_dns_delete_host(hc->host);	/* redo DNS */
...
static void client_commit_server(struct client_state *state)
{
	...
	if (conf_get_uint_v(3, CT_client, CT_server, CT_source) &&
	    cf->conf_server[0] != '\0') {
		...
		if (ada_conf.oem[0] && ada_conf.model[0] &&
		    !cf->conf_serv_override) {
			snprintf(hc->host, sizeof(hc->host),
			    CLIENT_SERVER_HOST_OEM_FMT, ada_conf.model,
			    ada_conf.oem, host_entry->domain);
		} else {
			snprintf(hc->host, sizeof(hc->host),
...
int client_set_region(const char *region)
{
	...
	strncpy(cf->region, new_host->region, sizeof(cf->region));
	return 0;
}
...
static void client_reset(struct client_state *state)
{
	...
	al_ada_timer_cancel(&state->cmd_timer);
	al_ada_timer_cancel(&state->poll_timer);
	al_ada_timer_cancel(&state->listen_timer);

	http_client_reset(hc, MOD_LOG_CLIENT);
	hc->client_send_data_cb = NULL;
	hc->client_err_cb = client_err_cb;
...
static void client_init_hw_id_default(struct ada_conf *cf)
{
	...
	mac = cf->mac_addr;
	if (!cf->hw_id[0]) {
		snprintf(cf->hw_id, sizeof(cf->hw_id),
...
void client_init(void)
{
	...
	state->callback_queue[CQP_HIGH] =
	    client_callback_queue_new(CLIENT_HIGH_QLEN);
	state->callback_queue[CQP_DEF] =
	    client_callback_queue_new(CLIENT_DEF_QLEN);
	state->callback_queue[CQP_LOW] =
	    client_callback_queue_new(CLIENT_LOW_QLEN);
	...
	for (cbh = client_cb_handlers, i = 0; i < ARRAY_LEN(client_cb_handlers);
	     cbh++, i++) {
		if (cbh->func) {
			al_ada_callback_init(&client_cb[i], cbh->func, state);
		}
	}
#ifdef ADA_BUILD_ANS_SUPPORT
...
#endif
	log_client_init();
	al_ada_callback_init(&state->next_step_cb, client_next_step, NULL);
	al_ada_callback_init(&state->notify_cb, client_notify_cb, NULL);
...
static void client_json_status_get(struct server_req *req)
{
	...
	server_put(req, "{\"DSN\":\"%s\","	/* "DSN" for compatibility */
	    ...
	    "\"rsa-ke\""
	    "]}",
	    ada_conf.dsn, ada_conf.dsn, conf_sys_model,
	    hc->host, sys_mac, state->connect_time,
	    al_clock_get_total_ms(), adap_conf_sw_version(),
		adap_conf_sw_build(),
...
static void client_json_time_get(struct server_req *req)
{
	...
	server_json_header(req);
	al_clock_get(&clk_src);
	server_put(req, "{\"time\":%lu,\"mtime\":%llu,\"set_at_mtime\":%lu,"
	    "\"clksrc\":%d,\"localtime\":\"%s\",\"timezone\":%d,"
	    "\"daylight_active\":%u,\"daylight_change\":%lu}",
	    utc_time, al_clock_get_total_ms(), clock_set_mtime,
	    clk_src, buf,
