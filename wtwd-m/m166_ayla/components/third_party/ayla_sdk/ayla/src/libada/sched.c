...
void sched_run_all(void)
{
	...
	al_ada_timer_cancel(&st->timer);
	al_clock_get(&clk_src);
	if (!st->enabled || !st->mod_scheds || clk_src <= AL_CS_DEF) {
		return;
	}
...
move_on:
	...
	al_ada_timer_set(&st->timer, (earliest_event - utc_time) * 1000);
