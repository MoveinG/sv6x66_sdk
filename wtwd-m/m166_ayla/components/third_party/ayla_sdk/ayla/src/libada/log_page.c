		    ...
		    const char *msg, void *arg), void *arg)
{
	 ...
	 */
	mbuf = al_net_udp_buf_alloc(LOG_SIZE);
	if (!mbuf) {
		return;
	}
	bp = mbuf;
	len = log_buf_get(snapshot, lines, bp, LOG_SIZE, 0);
	 ...
	 struct log_msg_head *head, const char *msg, void *arg)
{
	...
	modname = al_log_get_mod_name(head->mod_nr);
	server_put(req, "<tr>"
...
void log_page_get(struct server_req *req)
{
	...
	server_put(req,
	    ...
	    "<tr><td>up time<td>%llu ms"
	    "<tr><td>build<td>%s"
	    "</table>"
	    "<hr>\n",
	    time_buf, al_clock_get_total_ms(),
	    adap_conf_sw_build());
	server_put_pure(req,
