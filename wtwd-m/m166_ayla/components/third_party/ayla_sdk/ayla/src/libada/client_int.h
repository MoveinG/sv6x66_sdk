...
struct client_state {
	...
	u64 connect_time;		/* time of last connection */
	u8 retries;
	 ...
	 * Queues for requests.
	 */
	struct al_ada_callback *current_request; /* callback in progress */
	struct client_callback_queue *callback_queue[CQP_COUNT];
	...
	struct al_ada_callback next_step_cb; /* cb for next req */
	struct al_ada_callback notify_cb;
...
extern struct al_ada_callback ada_conf_reset_cb;
...
struct client_callback_queue;	/* opaque structure */
...
int client_callback_enqueue(struct client_callback_queue *nq,
		struct al_ada_callback *cb);

struct al_ada_callback *client_callback_dequeue(
		struct client_callback_queue *nq);
