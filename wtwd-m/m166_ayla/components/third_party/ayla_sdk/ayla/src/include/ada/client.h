 ...
 */
#define CLIENT_PROP_WAIT	200000	/* property wait, milliseconds */
...
enum client_conn_state {
	CS_DOWN,
	CS_DISABLED,	/*1 network up but client not enabled */
	CS_WAIT_CONN,	/*2 wait for connection to service */
	CS_WAIT_ID,	/*3 waiting to get client ID */
	CS_WAIT_INFO_PUT, /*4 waiting to put client info */
	CS_WAIT_CMD_PUT,  /*5 waiting to resp to cmd */
	CS_WAIT_GET,	/*6 waiting for GET response */
	CS_WAIT_LANIP_GET, /*7 waiting for GET response to lanip key */
	CS_WAIT_POST,	/*8 waiting for POST response */
	CS_WAIT_OTA_GET,  /* waiting for GET response of OTA patch fetch */
	CS_WAIT_OTA_PUT, /* waiting for send OTA status */
	CS_WAIT_CMD_STS_PUT, /*11 waiting to send belated cmd status */
	CS_WAIT_ECHO,	/*12 waiting for response for an echo post */
	CS_WAIT_EVENT,//13	/* waiting for event or polling interval */
